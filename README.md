...
It also includes the file `state_library.py`, which is a dictionary of interesting inital states in the E-M CR3BP. A simple representative L1 halo orbit is shown in the Earth-Moon synodic frame below using it's name in the state library, 'L1-Halo'. The 2D manifolds for an L1-Lyapunov orbit are shown below it, with the stable (green) and unstable (red) invariant manifolds.
